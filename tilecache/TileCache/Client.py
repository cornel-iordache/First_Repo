...
def seed (svc, layer, levels = (0, 5), bbox = None, padding = 0, force = False, reverse = False, delay = 0 ):
    ...
    for z in range(*levels):
        ...
        NUM_THREADS = 1
        if ((endY - startY) / stepY) <= NUM_THREADS:
            # We don't care for parallelization
            print endY
            fetch_tile_range(0, svc, layer, startX, endX, stepX, startY, endY, stepY, z, total, zcount, start)
            print 'Done'
        else:
            threads = []

            ys_per_thread = (endY - startY) / NUM_THREADS
            print "Ys", endY - startY, "ys_per_thread", ys_per_thread
            total = 0
            zcount = 0
            for thread_no in xrange(NUM_THREADS):
                min_y = (thread_no * ys_per_thread)
                max_y = min_y + ys_per_thread
                t_args = (
                    thread_no,
                    svc,
                    layer,
                    ...
                    endX,
                    stepX,
                    (thread_no * ys_per_thread),
                    (endY) if thread_no == (NUM_THREADS-1) else (max_y),
                    stepY,
                    z,
                    total,
                    zcount,
                ...
                #print "Start X", t_args[3], "End X", t_args[4], "Step X", t_args[5]
                #print "Start Y", t_args[6], "End Y", t_args[7], "Step Y", t_args[8]
                #print "Z", z
                #print '-----'
                t = threading.Thread(
                    target=fetch_tile_range,
                    args=t_args
                )
                t.start()
                threads.append((t, t_args))

            for t, the_args in threads:
                zcount += the_args[4] * the_args[7]
                #print "Thread %s finished\n[%.4fs : %.3f/s] %s/%s" % (the_args[0], total / (time.time() - start + .0001), zcount, ztiles)
                t.join()
